...
(deftest function-calls-converted-to-variables
  (testing (and #_(is (= [{:type :variable, :name "f(x + 1)"}] (lex "f(x + 1)")))
                #_(is (= [{:type :variable, :name "a"} {:type :and} {:type :variable, :name "f(x)"}] (lex "a && f(x)")))
                #_(is (= [{:type :variable, :name "f(x)"} {:type :and} {:type :variable, :name "f(y)"}] (lex "f(x) && f(y)")))
                #_(is (= [{:type :variable, :name "f(a && b)"}] (lex "f(a && b)")))
                #_(is (= [{:type :not} {:type :variable, :name "f(a && b)"}] (lex "!f(a && b)")))
                #_(is (= [{:type :not} {:type :open} {:type :variable, :name "f(a && b)"} {:type :or} {:type :variable, :name "b"} {:type :close}] (lex "!(f(a && b) || b)")))
                #_(is (= [{:type :variable, :name "f(g ( x ) + h ( y ))"}] (lex "f(g(x) + h(y))")))
                (is (= [] (lex "f(g(x) + h(y)")))
                (is (= [] (lex "a && f(g(x) + h(y)")))
                )))
